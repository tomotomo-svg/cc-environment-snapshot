...
## ⚡ 自動反映ルール（Skill/MCP/CLI 追加時）

Skill・MCP・CLI など環境に変化があったら、以下を必ず一気に実行する（確認なし）：

1. **数字を更新** — ヒーロー stat-chip・SVGノード・Section見出しの本数を実際の数に合わせる
2. **HTMLにカテゴリ追加** — Section 5（Skill）または Section 4（MCP）または Section 6（CLI）に新カードを追記。日付 `🆕 YYYY-MM-DD` を付ける
3. **MDファイル更新** — `current_development_environment.md` に同じ内容を追記、`updated` 日付を今日に変更
4. **git push** — `main` ブランチへ push → GitHub Pages が自動デプロイ（1分以内）
